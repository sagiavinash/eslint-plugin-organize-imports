  ...
  getModuleMeta: function({importNode, context, commentRules, pathAliases}) {
    ...
    var importPath = pathAliasConfig ? (
      importNode.source.value.replace(
        ...
        () => path.relative(sourceFilePath, path.join(process.cwd(), pathAliasConfig.resolvesTo))
      )
    ...
    const moduleType = (() => {
      if (resolvedPath.isLocalFile) {
        const commentRule = _.find(commentRules, (rule) => {
          const didMatchIncludePaths = (
            _.find(rule.include, (rulePath) => (
              _.startsWith(relativePath, rulePath) ||
              minimatch(relativePath, rulePath) ||
              minimatch(relativePath, `${rulePath}/**`)
            ))
          );
          const didMatchExcludePaths = (
            _.find(rule.exclude, (rulePath) => (
              _.startsWith(relativePath, rulePath) ||
              minimatch(relativePath, rulePath) ||
              minimatch(relativePath, `${rulePath}/**`)
            ))
