...
export default {
  ...
  fixOutOfOrder(context, firstNode, secondNode) {
    ...
    const firstRootEnd = this.findEndOfLineWithComments(sourceCode, firstRoot);
    ...
    let secondNodeCode = sourceCode.text.substring(secondRootStart, secondRootEnd);
    if (secondNodeCode[secondNodeCode.length - 1] !== '\n') {
      secondNodeCode += '\n';
    }

    let firstNodeCode = sourceCode.text.substring(firstRootStart, firstRootEnd);
    if (firstNodeCode[firstNodeCode.length - 1] !== '\n') {
      firstNodeCode += '\n';
    }
    ...
    return (fixer) => (
      fixer.replaceTextRange(
        ...
        secondNodeCode + sourceCode.text.substring(firstRootEnd, secondRootStart) + firstNodeCode,
      )
    );
  },
};
